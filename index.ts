...
(async () => {
	 ...
	 */
	let loadLibrarys = (res: resInfo, callback?: any) => {
		loadfiles(res, async (blob: Blob, name: string) => {
			if (blob.type === mimetypes.js) return eval(await new Response(blob).text());
	...
	loadfiles(
		...
		async (blob: Blob, name: string) => {
			...
			let destsHTML = navbar.dests.SP.map((destId: string) => (destId && `<div id="${destId}" class="fi-${destId.toLowerCase()} dest" title="${navbar.dests.en[destId] || ''}">${destId}</div>`) || `<div></div>`);
			document.getElementById('destbar')!.insertAdjacentHTML('beforeend', `<div class="dests">${destsHTML.join('')}</div>`);
		},
		0
	);
	// 目的地按钮点击事件
	document.getElementById('navdest')!.addEventListener('click', (e: Event) => {
	...
	window.addEventListener('mouseup', (e: Event) => {
		document.querySelector('.navtab')!.contains(e.target as HTMLElement) || document.querySelector('.head')!.classList.add('fold');
		if (document.getElementById('navdest')!.contains(e.target as HTMLElement)) return;
		if (document.querySelector('.dests')!.contains(e.target as HTMLElement)) return;
		document.getElementById('destbar')?.classList.add('hidden');
	});
	...
	loadLibrarys(cssIcons, async (filename: string) => {
		if (filename == cssIcons.file[0]) {
			let navtab = document.querySelector('.navtab') as HTMLElement;
